...
@RestController
@RequestMapping("/app")
public class ControllerStandar {
	...
	public ResponseEntity<Integer> inscribir(@PathVariable String ncarrera,@RequestBody Corredor c){
		...
		if(dorsal!=-1) {
			response=new ResponseEntity<Integer>(Integer.valueOf(dorsal),HttpStatus.ACCEPTED);
		}
		return response;
	}
}
